...
sts_grafico_alfa <- tibble::tibble(
  sts = paste0("β1 ± EP = ",
               ...
               "<br> F<sub>",
               sts_modelo_alfa$fstatistic[2] |> round(1),
               ", ",
               sts_modelo_alfa$fstatistic[3] |> round(1),
               "</sub> = ",
               sts_modelo_alfa$fstatistic[1] |> round(2),
               ", p = ",
               ...
               ", R² = ",
               sts_modelo_alfa$adj.r.squared |> round(2)),
  `Q = 1` = 4,
  `Distância da Borda` = df_alfa |>
    dplyr::pull(`Distância da Borda`) |> mean()) |>
  as.data.frame()

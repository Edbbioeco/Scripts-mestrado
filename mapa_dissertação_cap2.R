...
mapa_paralelo_cap2 <- ggplot(data = br) +
  geom_sf(color = "black", fill = "gray", linewidth = 0.5) +
  geom_sf(data = pe, color = "black", fill = "orange", linewidth = 0.5) +
  tidyterra::geom_spatraster_rgb(data = saltinho_rast) +
...
cowplot::ggdraw(mapa_principal_cap2) +
  cowplot::draw_plot(mapa_paralelo_cap2,
                     x = 0.05,
                     y = 0.15,
                     height = 0.35,
                     width = 0.35) +
...
ggsave(filename = "mapa_dissertacao_cap2.png", height = 10, width = 12)
